...
export class TeamDocumentsModal extends Modal {
	...
	onOpen() {
		...
		// Document list
		const listContainer = contentEl.createDiv({ cls: 'hivemind-document-list' });
		...
		if (this.documents.length === 0) {
			...
			const buttonContainer = contentEl.createDiv({ cls: 'hivemind-button-row' });
			...
			this.documents.forEach(doc => {
				const itemContainer = listContainer.createDiv({ cls: 'hivemind-document-item' });
		...
		// Footer buttons
		const footerContainer = contentEl.createDiv({ cls: 'hivemind-modal-footer' });
